...
function addOrder($id_order,$par_qui,$orderBuyer,$countryName,$countryCode,$partNumber,$codePochette,$quantity,$deliveredDate,$wip,$lien_couv,$family,$week,$year,$shortTerm,$model)
{
	$dataOuput = array('info' => "New order" ,'idOldOrder'=>0 );
	global $wpdb;
	//Check if project exist
	$_sql = "SELECT * FROM forecasts WHERE deliveredDate='".$deliveredDate."' AND family='".$family."' AND countryCode='".$countryCode."'
	AND partNumber='".$partNumber."' AND codePochette='".$codePochette."' AND wip<>'READY' AND wip<>'PREPARATION' AND shortTerm=".$shortTerm;
...
if(isset($filename)){
	...
	$data['result'] .= "<div id='long-term' style='display:none;margin-right:-10%;margin-left:-10%;padding:10px;background-color:#FFF;border:solid 1px;border-radius:5px;height:90vh;width:120%;overflow-x: auto;overflow-y: auto;white-space: nowrap;'>";
	...
	$r = 1;
	$model = "";
	$orderBuyer = "STELLANTIS"; //Par défaut si l'usine n'est pas détecté en cellule C1
	...
	foreach($sheet->getRowIterator() as $row) {
		...
		foreach ($row->getCellIterator() as $cell) {
			...
			if($x>2 && array_search($x,$tabcolVide)===false) {
				...
				if($r == 1 && strlen($value) > 3)
				{
					if(stristr($value,"-"))
					{
						$model = explode("-",$value)[0];
						$orderBuyer = explode("-",$value)[1];
					}
					else
					$model = $value;

					$model=ltrim($model); //Del espace à gauche
					$model=rtrim($model); //Del espace à droite
					$orderBuyer=ltrim($orderBuyer); //Del espace à gauche
					$orderBuyer=rtrim($orderBuyer); //Del espace à droite
				}
			...
			if($r > $rEntete){
				...
				else if ($x > $cCodePochette)
				{
					...
					if(array_key_exists($x, $tabWeek) && array_key_exists($x, $tabYear)){
						...
						$tab_info_id= addOrder($id_order,$par_qui,$orderBuyer,$countryName,$countryCode,$partNumber,$codePochette,$value,$datesem,"PREPARATION","https://mdw-05.fr/wp-content/uploads/2021/01/307.png",$family,$tabWeek[$x],$tabYear[$x],0,$model);
	...
	//////////////////// ANALYSE COURT TERME ///////////////////
	$data['result'] .= "<div id='short-term' style='margin-right:-10%;margin-left:-10%;padding:10px;background-color:#FFF;border:solid 1px;border-radius:5px;height:90vh;width:120%;overflow-x: auto;overflow-y: auto;white-space: nowrap;'>";
	$data['result'] .= "<table class='tableaulong'><thead>";
	$condRefExistePAS = false;
	$rEntete = 200;
	$cPartNumber = -1;
			...
			if($r > $rEntete){
				...
				else if ($x > $cCodePochette)
				{
					...
					if(array_key_exists($x, $tabDate)){
						...
						$tab_info_id= addOrder($id_order,$par_qui,$orderBuyer,$countryName,$countryCode,$partNumber,$codePochette,$value,$datesem,"PREPARATION","https://mdw-05.fr/wp-content/uploads/2021/01/307.png",$family,$deliveredWeek,substr($tabDate[$x],6,4),1,$model);
						if($tab_info_id['idOldOrder'] > 0 )
						$sidOldOrder .= $tab_info_id['idOldOrder'] . ",";
					}
				}
