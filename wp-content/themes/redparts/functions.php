...
//  $sheet = $objPHPExcel->getSheet(0);
  ...
  // if($x == 2) //On détecte de le numéro $dosdetID
  // {
  // $dosdetID = $cell->getValue();
  //
  //
  // if($dosdetID != "DOSDETID")
  // {
  // $Req_sql = "SELECT * FROM details_manuels WHERE DOSDETID=".$dosdetID;
  // $tabResultsappareil = $wpdb->get_results($Req_sql, ARRAY_A); //On rempli le tableau avec les données lu
  // if($wpdb->num_rows == '0')
  // {//l'affaire n'existe pas, on peut ajouter
  //
  // $tabvalues = array();
  // // On boucle sur les cellule de la ligne pour compléter un tableau
  // foreach ($row->getCellIterator() as $cell) {
  // $valuecell = $cell->getValue();
  // if($valuecell > 36500 && $valuecell < 50000)
  // {
  // $tabvalues[] = \PHPExcel_Style_NumberFormat::toFormattedString($valuecell, 'DD-MM-YYYY');
  // }
  // else {
  // $tabvalues[] = $cell->getValue();
  // }
  // }
  // //On a les données, on peut les ajouter
  // $wpdb->insert("details_manuels", array(
  //   "DOSSIERID" => $tabvalues[0],
  //
