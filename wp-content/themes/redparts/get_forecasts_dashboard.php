...
{
  $_sql = "SELECT * FROM forecasts WHERE wip <> 'PREPARATION' AND deliveredDate >= '".$dateDebut."' AND deliveredDate <= '".$dateFin."' ORDER BY orderBuyer ASC, model ASC, deliveredDate ASC";
...
foreach ($tabOrderbyWeek as $keyO => $result) {
  if($result['orderBuyer']. " - " . $result['model'] != $temp_orderBuyer_model)
  {
    if($colorModel=="grey")
    {
      $colorModel="#CDCD00";
    ...
    $bodyTable .= "<tr style='background-color:".$colorModel.";color:white'><td style='text-align:left;font-weight:bold;' colspan='5'>".$temp_orderBuyer_model."</td></tr>";
  }
...
if($O > 1)
$data['orders'] = str_replace("%ORDER%",$O. " Forecasts",$data['orders']);
else {
  $data['orders'] = str_replace("%ORDER%",$O. " Forecast",$data['orders']);
}
